# src/domain/evaluator — suite doc
...
`forge-test --run src/domain/evaluator`; a single file: `forge-test --run src/domain/evaluator/<file>.test.js`. The same directory also holds the
...
- Nothing is mocked or stubbed anywhere in this suite. A file's catalogue is
  ...
  from the frozen fixtures at `src/domain/evaluator/__fixtures__/whfb6-definitive`
