...
describe('classifyStandaloneOption', () => {
  it('marks min===max (min>0) as mandatory', () => {
    expect(classifyStandaloneOption({ minLimit: 1, maxLimit: 1 })).toEqual({ isMandatory: true, isBinary: true });
  });

  it('marks max===1 as binary', () => {
    expect(classifyStandaloneOption({ minLimit: 0, maxLimit: 1 })).toEqual({ isMandatory: false, isBinary: true });
  });

  it('is neither mandatory nor binary for an unbounded max', () => {
    expect(classifyStandaloneOption({ minLimit: 0, maxLimit: Infinity })).toEqual({ isMandatory: false, isBinary: false });
  });
});
