...
- A `useRoster` file that drives the hook through the **production seam** —
  ...
  (`src/domain/roster/createRoster.js`); nothing about the roster or the catalogue is
  ...
  from `src/domain/evaluator/__fixtures__/whfb6-definitive/`) for the two shapes this
...
- `addUnit(entry, categoryId, targetForceId?)` is the hook's own recruitment
  ...
  call directly (that belongs in `src/domain/roster`).
