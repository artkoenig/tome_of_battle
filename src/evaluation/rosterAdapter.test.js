 ...
 * (`src/evaluation/rosterAdapter.js`, existiert noch nicht; test-first).
 *
 * Der Adapter uebersetzt das App-Roster (IndexedDB-Modell, `src/types.js`) in
 * den Eingabevertrag der Evaluator-Fassade (`src/evaluator/evaluator.js`,
...
// ── App-Roster-Bausteine (Shape aus src/types.js) ───────────────────────────

/** Eine App-Selection mit allen Pflichtfeldern aus `src/types.js`. */
...
/** Eine App-Force mit allen Pflichtfeldern aus `src/types.js`. */
function appForce({ id, forceEntryId, catalogueId = 'cat-main', selections = [] }) {
  return { id, forceEntryId, catalogueId, selections };
}

/** Ein vollstaendiges App-Roster mit allen Pflichtfeldern aus `src/types.js`. */
function appRoster({
  costLimit = 1500,
