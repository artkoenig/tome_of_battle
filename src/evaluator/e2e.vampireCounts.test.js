/**
 * Reale E2E-Szenarien der **Vampire-Counts**-Armee (ADR-0032): das Roster wird
 * end-to-end gegen die echten Definitive-Edition-Daten ausgewertet — die `.gst`,
 * die Vampire-Counts-`.cat` **und** ihre gemeinsame `Mercenaries`-Abhaengigkeit.
 *
...
import {
  vampireCountsDataset,
  vampireCountsDatasetWithoutMercenaries,
  VAMPIRE_COUNTS_FORCE_ID,
  VAMPIRE_COUNTS_CORE_UNIT_IDS,
...
const emptyForce = () => roster(force(VAMPIRE_COUNTS_FORCE_ID, []));

/** Eine bekannt-regelkonforme Liste: General-Aufwertung + zwei Core-Einheiten. */
const conformingArmy = () =>
  roster(force(VAMPIRE_COUNTS_FORCE_ID, [
    selection(GENERAL_DESIGNATOR_ID),
    ...VAMPIRE_COUNTS_CORE_UNIT_IDS.map(id => selection(id)),
  ]));

describe('E2E Vampire Counts: armeeweite Pflichtregeln General und Core', () => {
  it('schlaegt bei leerem Kontingent fuer General (min 1) und Core (min 2) an', () => {
    const report = evaluate(vampireCountsDataset(), emptyForce());
  ...
  it('erzeugt fuer eine bekannt-regelkonforme Liste keine falsche General- oder Core-Verletzung', () => {
    const report = evaluate(vampireCountsDataset(), conformingArmy());
...
describe('E2E Vampire Counts: kataloguebergreifende Auflösung ueber Mercenaries', () => {
  ...
  it('meldet die fehlende Mercenaries-Abhaengigkeit als Diagnose und laesst ihren Verweis baumeln', () => {
    const report = evaluate(vampireCountsDatasetWithoutMercenaries(), emptyForce());
