...
- **Versioning + changelog** (`versionPlugin` in `vite.config.js`, pure logic in `scripts/versioning.js`, single source of truth for both the tag and `changelog.json`): every `vite build` computes a semver version — building `main` cuts the next **minor** release (`vX.(Y+1).0`) and tags+pushes it; building any other branch is **not** a release and instead appends the commit hash as build metadata (`vX.Y.Z+<shorthash>`, no tag). The **major** is only ever set manually. `changelog.json` notes = commit subjects since the newest release tag up to HEAD. The update toast fetches `changelog.json` fresh (cache-busted) and shows "what's new". Opt-outs: `SKIP_AUTO_TAG` disables tagging, `AUTO_TAG_NO_PUSH` keeps the tag local. The production build/tag runs in `.github/workflows/build-release.yml` on push to `main`.
