...
- After any UI-visible change, take a Puppeteer screenshot of the affected view (before/after or just the result) and send it to the user as confirmation — but only when the user is not working locally at their own machine (e.g. a remote/cloud session such as claude.ai/code web or mobile), since in that case they can't otherwise see the running app themselves. Skip this when the session runs on the user's local machine — they can check the UI themselves there.
