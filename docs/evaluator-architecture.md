...
**Unverlinkte Kategorien.** Grenzen können direkt an der `categoryEntry` hängen und gelten auch ohne wiederholenden `categoryLink` (Datenformat §5.5/§5.6). Führt ein Kontingent die Kategorie per `categoryLink`, hängt ihr Anker dort; führt sie **kein** Kontingent, bekommt sie ihren Anker allein aufgrund ihrer Grenzen — eine MIN-Grenze über das Pflicht-Phantom, eine Kategorie mit **ausschließlich** MAX-Grenzen (die 0–1-Kodierung) über einen eigenen Kategorie-Anker: armeeweit skopierte Grenzen an der Wurzel, kontingent-skopierte **einmal** am ersten Kontingent (die Ziel-Typ-Regel §4.5 zählt sie ohnehin armeeweit; je ein Anker pro Kontingent meldete dieselbe Verletzung mehrfach).
