...
**Träger.** Ein Modifikator wirkt auf das Element, an dem er hängt — den Knoten selbst *oder* eines seiner Info-Elemente (Profil, Regel, Info-Gruppe, Info-Verweis), denn die `EntryBase` der XSD gibt allen dieselben `modifiers`. Das ist der Normalfall, nicht die Ausnahme: in den Fixture-Katalogen der 6th Definitive Edition hängen **359 der 360** Charakteristik-Modifikatoren an einem `<profile>` (48) oder an einem `<infoLink>` (311) — **genau einer** hängt an einer `selectionEntry`, und keiner an einem `entryLink`, einer Gruppe, einer Kategorie oder einem Kontingent. Der eine Ausreißer steht in `Forces of Chaos (6th definitive edition).cat`: die `selectionEntry` „Champion" trägt unter einer `atLeast`-Bedingung ein `<modifier type="increment" value="1" field="6b9f-c8fe-8998-27e3">` auf den in der `.gst` deklarierten Charakteristik-Typ „A" — unmittelbar neben einem gleichlautenden Modifikator am eigenen `infoLink` „Champion" (`c735-563a-a91e-7513`) desselben Eintrags. Damit ist die Frage, ob ein Merkmals-Modifikator *alle* Profile eines Knotens mit diesem Charakteristik-Typ trifft oder nur eines, aus den Daten beantwortet: er trifft **genau das Profil, an dem er steht** — der Knoten ist nie sein Träger. Führt ein Knoten mehrere Profile mit demselben Charakteristik-Typ, bleiben die anderen unberührt. Sichtbarkeit, Name und Merkmale schlüsseln entsprechend nach dem Paar (Knoten, Träger); Kosten, Kategorien, Grenzen und Meldungen bleiben am Knoten. Bedingungen werden immer im Query-Kontext des tragenden **Knotens** ausgewertet — nur er hat eine Position im Baum. Der erweiterte Fixture-Korpus hält zu dieser Absolutheit genau einen Gegenfall, den oben genannten Modifikator an der `selectionEntry` „Champion"; er ist in Issue 0148 (INC-4) festgehalten und dem Maintainer überlassen — die Entscheidung wird hier nicht revidiert.
