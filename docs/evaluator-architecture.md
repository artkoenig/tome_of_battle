...
**Eine in beiden Wurzelformen geführte Pflichteinheit meldet genau einmal** (`report.js`, Datenformat §9.9). Führt ein Katalog dieselbe Pflicht als Wurzel-`selectionEntry` *und* als Wurzel-`entryLink` auf genau diesen Eintrag, bekommen im fehlenden Fall beide Formen ihren Pflicht-Anker — mit **verschiedenen** Grenz-Ids, die Entdopplung der Kategorie-Grenzen (oben, Schlüssel `(Grenz-Id, gezählte Ziel-Id)`) trennt sie also. Diese zweite Regel schlüsselt deshalb auf **aufgelöste Ziel-Id plus Rahmen**: die aufgelöste Ziel-Id ist die einzige, die beide Formen teilen, und der Rahmen ist die **konkrete Instanz** — zwei leere Kontingente sind zwei offene Pflichten, nicht eine. Zugeschnitten ist sie auf Pflicht-Anker an der Wurzel oder in einem Kontingent, deren Definition ein Eintrag oder ein Verweis auf einen Eintrag ist, mit einer `roster`- oder `force`-skopierten Grenze. Es überlebt die **erste** Meldung in Berichtsreihenfolge — dieselbe Vorzugsregel wie oben, zwei Entdopplungen mit gegenläufiger Präferenz wären eine Falle. Verworfen werden nur die Meldungen der weiteren *Anker*; trägt ein Anker mehrere Grenzen, bleiben sie vollständig. Die Ergebnisse und damit die Fähigkeitsdatensätze beider Slots bleiben unberührt.
