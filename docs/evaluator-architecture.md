...
**Wurzel-`entryLink` als Pflichtform.** Die armee- bzw. kontingentweite Pflichteinheit kommt in **zwei** gleichwertigen Kodierungen vor (Datenformat §9.9): der `min`-Constraint hängt an einem Wurzel-`selectionEntry` — oder an einem **Wurzel-`entryLink`**, dessen Ziel im geteilten Pool liegt. Die zweite Form bekommt einen eigenen Pflicht-Anker (`resolver.js`, `rootEntryLinks` → `evalTree.js`), verankert wie die erste: an der Wurzel für eine `roster`-Grenze, in **jedem** Kontingent für eine `force`-Grenze, und nur dort, wo die Einheit fehlt — gezählt unter der Link-Id **und** der aufgelösten Ziel-Id, denn ein Roster kann dieselbe Einheit unter beiden führen. Der Link selbst bleibt dabei **außerhalb** der Wurzel-Definitionsliste: ihn dort aufzunehmen ließe die Traversierung in seine Kinder absteigen und synthetisierte genau die falschen Pflichten, die ADR-0032 ausschließt. Der Anker ist doppelt zugeschnitten — auf seinen Rahmen und auf die **eigenen** Grenzen des Links: §9.9 wertet an dieser Form „die Constraint und die Modifier des Links" aus, „nicht die des Ziels", eine eigene `min`-Grenze des Ziels darf also nicht mitfeuern (sonst entstünden zwei Verstöße, wo einer richtig ist). Maßgeblich für die Verankerung ist die **Existenz** einer eigenen MIN-Grenze im Rahmen, nicht ihr Basiswert: die reale Kodierung ist Basis `min="0"`, per Link-`modifier` bedingt angehoben — greift die Bedingung nicht, ist `0 >= 0` erfüllt und der Anker meldet nichts. Ein Wurzel-`entryLink` auf eine `selectionEntryGroup` bleibt außen vor (eigene Lücke, Issue 0107).
...
**Eine in beiden Wurzelformen geführte Pflichteinheit meldet genau einmal** (`report.js`, Datenformat §9.9). Führt ein Katalog dieselbe Pflicht als Wurzel-`selectionEntry` *und* als Wurzel-`entryLink` auf genau diesen Eintrag, bekommen im fehlenden Fall beide Formen ihren Pflicht-Anker — mit **verschiedenen** Grenz-Ids, die Entdopplung der Kategorie-Grenzen (oben, Schlüssel `(Grenz-Id, gezählte Ziel-Id)`) trennt sie also. Diese zweite Regel schlüsselt deshalb auf **aufgelöste Ziel-Id plus Rahmen plus effektiven Grenzwert**: die aufgelöste Ziel-Id ist die einzige, die beide Formen teilen; der Rahmen ist die **konkrete Instanz** — zwei leere Kontingente sind zwei offene Pflichten, nicht eine; und der effektive Grenzwert gehört zum Schlüssel, weil zwei **verschiedene** Grenzwerte zwei verschiedene Aussagen sind (verlangt der Wurzel-Eintrag 1 und der Wurzel-Link 2, bleiben **beide** Meldungen stehen — „0 von 1" zu melden und „0 von 2" zu verschlucken ließe den Nutzer die gemeldete Grenze erfüllen, während die Liste illegal bliebe). Zugeschnitten ist sie auf Pflicht-Anker an der Wurzel oder in einem Kontingent, deren Definition ein Eintrag oder ein Verweis auf einen Eintrag ist, mit einer `roster`- oder `force`-skopierten Grenze. Bei **gleichem** Grenzwert überlebt die **erste** Meldung in Berichtsreihenfolge — dieselbe Vorzugsregel wie oben, zwei Entdopplungen mit gegenläufiger Präferenz wären eine Falle. Verworfen werden nur die Meldungen der weiteren *Anker*; trägt ein Anker mehrere Grenzen, bleiben sie vollständig. Die Ergebnisse und damit die Fähigkeitsdatensätze beider Slots bleiben unberührt.
