...
| `selectionIdentityId` | ~~`defIdOf`~~, ~~`optionDefinitionIdOf`~~, ~~`entryLinkId \|\| selectionEntryId`~~ | armylist, ruleengine, ui, shared | The id that **identifies** a selection: the link it was set through, else the entry it names. Holds after `reconcileImportedSelectionIds` (ADR 0011 §2), i.e. everywhere the app carries a roster. Defined once, in `src/shared/rostermodel/selectionIds.js` — the order of the two ids is never spelled out inline again. |
| `importedCatalogueEntryId` | ~~`catalogueEntryIdOf`~~, ~~`selectionEntryId \|\| entryLinkId`~~ | armylist, shared | The catalogue entry a **raw, not-yet-reconciled** import refers to: the target the `.ros` names, else its link. The pre-reconciliation counterpart of `selectionIdentityId`, and the only order that finds a linked option during import. Same home. |
