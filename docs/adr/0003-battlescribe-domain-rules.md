...
- **Geteilte und nicht geteilte Queries (`shared`):** Das Attribut `shared` einer Query — also einer Constraint *oder* einer Condition (XSD `QueryBase`) — bestimmt ihren Bezugsrahmen:
  ...
  - `shared="false"`: gezählt wird **nur die eine Instanz**, an der die Query hängt. `scope` und `includeChildSelections` behalten ihre Bedeutung: gezählt wird im Teilbaum dieser Instanz, und `includeChildSelections` entscheidet wie überall sonst, ob unterhalb der Instanz weitergezählt wird.
  - Der `scope="parent"` bleibt davon unberührt — er ist bereits an genau eine Instanz (den Eltern-Container) gebunden.
...
- **Mutually-Exclusive Choices (Radio-Buttons):** Auswahlgruppen (Selection Groups) mit `max="1"` stellen eine sich gegenseitig ausschließende Auswahl dar (Radiobutton-Semantik), nicht die quantitative Einschränkung eines zählbaren Gegenstandes auf maximal 1.
  - **Max-hebbar ⇒ Mehrfachauswahl:** Maßgeblich für die Radio-vs-Checkbox-Entscheidung ist nicht der rohe `max="1"`, sondern ob ein Modifier das Gruppen-Max über 1 **heben kann**. Trägt die Gruppe einen (nicht-`<repeat>`) Modifier, der ihr Max über 1 hebt — der klassische Fall ist ein bedingter `increment` auf die Gruppen-Max-Constraint, gekoppelt an eine andere Auswahl (Rüstung **+** Schild) —, rendert sie als **Mehrfachauswahl mit Live-Zähler**, und zwar **schon bevor** die Bedingung erfüllt ist. Am *aktuellen* effektiven Max festzumachen erzeugte einen Teufelskreis (ohne Schild wäre das Max 1 → Radio → das Schild ließe sich nie anwählen). Nur eine **echt fix auf `max=1`** gedeckelte Gruppe **ohne** solchen Modifier bleibt Radio. Der senkende Fall (Max bedingt → 1: Ausschluss) und die Deaktivierung (Max bedingt → 0) leiten sich dagegen aus dem *aktuellen* effektiven Max ab. Die statische „hebbar?"-Erkennung ist `canGroupMaxBeRaisedAboveSingleChoice`; das `increment`+`<repeat>`-Muster (mehrere Stück desselben Items, Dispel Scroll) bleibt davon unberührt und wird gesondert als Mengen-Stepper behandelt. Sämtliche Auswahl-, Anzeige- und Recruit-Entscheidungen leiten sich aus den **effektiven** (modifier-angepassten) Constraint-Werten ab, nie aus rohen. Details und XML-Beispiel: [`docs/battlescribe-data-format.md` §9.8](../battlescribe-data-format.md#98-bedingter-modifier-auf-ein-gruppen-maxmin-an-eine-andere-auswahl-oder-einen-scope-gekoppelt).
