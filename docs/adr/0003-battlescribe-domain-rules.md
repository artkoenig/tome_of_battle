...
- **Auswahl-Kategorien in der UI:** Die Kategorie mit `primary="true"` bestimmt, in welcher Kategorie-Gruppe (Sektion) eine Einheit in der UI einsortiert wird. Kategorien mit `primary="false"` dienen als unsichtbare Schlagworte für Validierungsregeln (z. B. zur Einschränkung, wer ein Reittier wählen darf). Die UI darf Einheiten niemals nach hardcodierten Kategorienamen gruppieren.
  ...
  - **Listenkonfigurationen sind keine Einheiten:** Eine `selectionEntry` vom Typ `upgrade`, deren gesamter Teilbaum profil- und kostenlos ist und die direkt an der Armeeliste hängt (siehe `CONTEXT.md`, Begriff „Listenkonfiguration"), wird nie als Einheit gruppiert oder gerendert. Das Kriterium ist datengetrieben (Schema-Typ + Struktur), nicht auf Kategorienamen oder Kataloge hartkodiert — es gilt für jede Datenquelle gleich (ergänzt in Main-Issue 34). Dasselbe Strukturkriterium gilt gleichwertig für eine rohe Katalog-Eintragsdefinition, noch bevor der Spieler sie gewählt hat (siehe `CONTEXT.md`, Begriff „Listenkonfigurations-Kategorie"): eine Kategorie, deren sichtbare Katalog-Einträge ausschließlich Listenkonfigurationen sind, rendert im Editor immer als aufklappbare Konfigurations-Kachel statt über den Aushebe-Dialog — auch mit 0 Auswahlen (ergänzt in Main-Issue 35).
