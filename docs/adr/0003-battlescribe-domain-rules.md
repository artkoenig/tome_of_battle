...
- **Mutually-Exclusive Choices (Radio vs. Mehrfachauswahl):** Die Gruppen-Semantik (`max="1"` als Radio, „Max-hebbar ⇒ Mehrfachauswahl" samt Teufelskreis-Argument und XML-Beispiel) steht in der [BSData-Doku](../battlescribe-data-format.md) (§9.2, [§9.8](../battlescribe/patterns/common-patterns.md#98-bedingter-modifier-auf-ein-gruppen-maxmin-an-eine-andere-auswahl-oder-einen-scope-gekoppelt)). Entscheidungen hier: die statische „hebbar?"-Erkennung ist `canGroupMaxBeRaisedAboveSingleChoice`; das `increment`+`<repeat>`-Muster (mehrere Stück desselben Items, Dispel Scroll) wird gesondert als Mengen-Stepper behandelt; sämtliche Auswahl-, Anzeige- und Recruit-Entscheidungen leiten sich aus den **effektiven** (modifier-angepassten) Constraint-Werten ab, nie aus rohen.
