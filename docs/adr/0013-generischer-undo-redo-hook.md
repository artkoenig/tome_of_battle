...
- **Option 1:** Generischer Hook `src/ui/viewmodels/useUndoableState.js`, der einen beliebigen State kapselt (`{ state, setState, replace, undo, redo, canUndo, canRedo }`). `useRoster.js` nutzt ihn anstelle von `useState` für den `roster`-State.
