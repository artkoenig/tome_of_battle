...
**`package.json#version` ist normativ, jede andere versionstragende Stelle ist abgeleitet.** `package-lock.json` führt die Version an zwei Feldern (oberste Ebene und Eintrag des Wurzel-Pakets) als Spiegel und wird vom Versions-Bump mitgezogen — nicht von Hand und nicht von einem beliebigen späteren `npm install`, das die Abweichung sonst stillschweigend kaschiert. Ein Bump, der nur einen Teil der Stellen erreicht, bricht sichtbar ab. Daraus folgen zwei Regeln: `npm pkg set version=…` ist als Bump **verboten**, weil es die Lockfile unberührt lässt und damit genau diese Lücke wieder aufreißt; und ein neues Artefakt, das die Version trägt, wird entweder zur Bauzeit aus `package.json` abgeleitet oder als abgeleitete Stelle registriert, damit dieselbe Prüfung es mitbewacht. Bei einem Merge-Konflikt wird `package.json` aufgelöst und der Bump erneut ausgeführt; die Versionsfelder der Lockfile werden nie von Hand gemergt.
