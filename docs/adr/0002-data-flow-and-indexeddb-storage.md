...
1. **Import:** `Importer.jsx` beschafft lediglich die Roh-XML — beim Datei-Upload über `src/data/parser/zipExtractor.js` (Entpacken von `.bsz`/ZIP-Dateien), beim Bundle-Import über die Katalogquellen aus `src/data/db/catalogSourceIndex.js`. Der eigentliche Abschluss des Imports liegt für beide Wege gemeinsam in `src/data/db/systemImport.js`: Schema-Hinweis, Parsen über `src/data/parser/xmlParser.js` in bereinigte JavaScript-Systemobjekte (Kataloge, Kategorien, Profile, Regeln, Einschränkungen etc.), Prüfung fehlender Bibliothekskataloge (`src/data/parser/libraryDependencies.js`) und Speichern.
2. **Datenzugriff kapseln (Repository Pattern):** Direkte Zugriffe auf die IndexedDB (mittels nativer API oder Wrappern) dürfen *nicht* in React-Komponenten stattfinden. Die Datei `src/data/db/database.js` dient als einzige Schnittstelle zur IndexedDB (Object Stores: `systems`, `rosters` und `settings`). Sie bietet Promise-basierte Wrapper für Lese- und Schreibvorgänge an.
...
5. **Autosave & Validierung (Roster State):** `src/ui/viewmodels/useRosterState.js` verwaltet die Auswahlliste. Jede Änderung triggert eine immutable Statusänderung. **Nur das Schreiben in die IndexedDB ist um 150ms debounced**; ausstehende Speicherungen werden beim Unmount geflusht. Kosten und Verletzungen sind demgegenüber **synchrone `useMemo`-Ableitungen** aus Roster und System über den Bericht der Reinraum-Engine (`src/domain/evaluation/useEvaluation.js`, seit Issue 0121; zuvor `src/solver/validator.js`) — sie stehen unmittelbar mit der Änderung bereit und nicht erst nach Ablauf der Verzögerung. Das ist Voraussetzung dafür, dass abgeleitete Anzeigen wie die Verfügbarkeit im Aushebe-Dialog (ADR-0035) keine veralteten Werte zeigen.
6. **Migrations-Pipeline:** Beim App-Start (angestoßen aus dem Hook `src/ui/viewmodels/useAppData.js`) prüft `src/data/db/migrations.js` (`runSystemMigrations`) alle in der IndexedDB gespeicherten Systeme und führt bei Bedarf strukturelle Upgrades durch, um die Abwärtskompatibilität älterer Roster zu gewährleisten.
