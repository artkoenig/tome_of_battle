...
- Die Anwendung verzichtet auf einen externen Router (wie React Router). Die Navigation zwischen den Hauptansichten (`rosters`, `importer`, `builder`, `play`) wird als einfacher Zustand (View-Switcher) verwaltet — gekapselt im Hook `src/ui/hooks/useAppNavigation.js`, den `App.jsx` verdrahtet (Navigation bleibt App-Zustand, kein Router). Dies hält die PWA leichtgewichtig.
