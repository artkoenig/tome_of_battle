...
- AC5 Die Szenarien `docs/testing/violation-classification` und `docs/testing/offer-and-category-slots` erwarten den Manbiters-Wurzel-Angebots-Anker (`0efb-7f63-7932-0655`) in einem Vampirfürsten-Kontingent, obwohl kein Vampirfürsten-Katalog diesen Link deklariert. Diese Erwartung ist nach der neuen Regel falsch und wird aus den Katalogdaten heraus korrigiert; jede übrige gepinnte Aussage der beiden Szenarien bleibt unangetastet. | verify: forge-test --run src/evaluator
...
- AC7 Alle Checks grün. | verify: forge-test && forge-lint && forge-typecheck && forge-build
