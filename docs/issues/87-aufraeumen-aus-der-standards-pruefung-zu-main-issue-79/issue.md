...
- Zusatz aus der Umsetzung von Issue 84, passt zu Punkt 3 dieses Issues: seit 354dc69 liefert der Schema-Codegen die von der XSD deklarierten Attribut-Vorgaben als AttributeDefault (12 Eintraege). Damit sind DEFAULT_SHARED_QUERY (xmlParser.js:203) und die false-Vorgaben in getBooleanAttribute Handkopien dessen, was generiert bereitsteht — AttributeDefault.SHARED ist 'true', AttributeDefault.HIDDEN ist 'false'. Wer Punkt 3 anfasst, sollte DEFAULT_SHARED_QUERY nicht bloss auf seine Datei beschraenken, sondern pruefen, ob es ueberhaupt noch von Hand gefuehrt werden muss. Dieselbe Drift-Klasse, die Issue 84 fuer die Dezimal-Vorgabe geschlossen hat, steht fuer die Boolean-Vorgaben noch offen.
- Weiterer Punkt fuer dieses Aufraeumen, aus der Standards-Achse zu Main-Issue 82: serializeSelection (src/utils/rosterSerialization.js:146) nimmt sechs Stellungsparameter, hat genau eine Aufrufstelle (:127) und bekommt von ihr immer dieselben Konstanten — drei der sechs Parameter sind tote Flexibilitaet. parentSelection wird zusaetzlich innerhalb der Faltung von context.parentSelection verdeckt (:153, :158). Gleiche Klasse wie Punkt 3 dieses Issues.
