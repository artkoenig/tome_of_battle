...
- PO-Entscheid zu einer Abweichung vom Modul-Plan (festgehalten, weil design.md am Gate geloescht wird): Der Plan fuehrte resolver.js ausdruecklich unter 'nicht angefasst'. Scheibe 04 hat dort dennoch collectGroupMemberIds auf identity.js umgestellt und damit eine neue Modulkante resolver.js -> identity.js geoeffnet. Das wird angenommen, nicht zurueckgebaut: der Plan selbst begruendet in seinem Befund 4 und in der Schluesselentscheidung 'Fuenf Stellen stellen dieselbe Frage', dass die Identitaets-Formel an einer Stelle stehen soll — er hatte diese fuenfte Kopie nur uebersehen. Die Aenderung schliesst genau die Luecke, fuer die der Plan argumentiert. Gleiches gilt fuer die Kante report.js -> identity.js: dort wurde eine wortgleiche lokale Kopie derselben Formel geloescht, keine neue Bedeutung eingefuehrt. Die beiden zusaetzlichen Ausfuhren von identity.js (resolvedTargetIdOf, hasCountableIdentity) sind additiv und liegen im erklaerten Zweck des Moduls.
- Aufloesungs-Gate: Fuenf Achsen geprueft, in zwei Runden plus einer Abschlusspruefung. Standards: beide harten Gates gruen, kein einziger Treffer der Export-Pruefung in src/evaluator/; die Achse fand in drei aufeinanderfolgenden Runden Befunde im eigenen Code dieses Main-Issues, alle in den Scheiben 04 und 05 behoben, die vierte Runde wurde als Issue 88 ausgelagert. Spezifikation: alle Kriterien von 76 UND von 78 erfuellt, 113/113 E2E-Szenarien gruen, die Erwartungsaenderung an modifier-characteristic-value aus den Katalogdaten belegt; dieselbe Achse hat den Messgroessen-Defekt unabhaengig von der Standards-Achse gefunden. Tests: gruen, 2174 vitest-Tests plus Puppeteer-E2E. Doku: 0 Funde nach Behebung zweier Befunde (fehlende Testkatalog-Eintraege, Zeitform im Szenario-README). Design-Konformitaet: Vertraege K1-K5 eingehalten; die Abweichung zu resolver.js ist am Issue begruendet angenommen.
