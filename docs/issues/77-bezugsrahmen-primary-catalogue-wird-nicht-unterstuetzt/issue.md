...
- KORREKTUR des vorstehenden PO-Entscheids. Die Zuschreibung dort ist falsch: der Implementierer hat die Verallgemeinerung des fail-closed NICHT einseitig entschieden. Sie stand als Akzeptanzkriterium 4 im Kind-Issue 03 ('Ein nicht aufloesbarer Bezugsrahmen wird generell nicht mehr als erfuellte Bedingung gelesen') und ebenso im Auftrag an ihn — beides vom Product Owner formuliert. Der Widerspruch bestand zwischen design.md (Risiko R4: hier nicht mitziehen) und dem Akzeptanzkriterium des Issues, also zwischen zwei Vorgaben des Product Owners, nicht zwischen Vorgabe und Umsetzung. Der Implementierer hat getan, was das Issue verlangte, und die Tragweite offengelegt und gemessen. Die sachliche Bewertung des Entscheids bleibt unveraendert — die Abweichung wird angenommen, aus den dort genannten Gruenden.
- Zwei kleinere Befunde der Spezifikations-Pruefung, beide ohne Aenderungsbedarf am Code: (1) Die eine Zeile, in der die Umsetzung von Kontrakt C4 abweicht (bei nicht unterstuetztem Feld liefert sie den Sentinel statt der geplanten 0), wurde begruendet und getestet, aber nicht wie die R4-Abweichung als eigener Plan-Abweichungs-Vermerk festgehalten. Hiermit nachgeholt: die Begruendung traegt und ist dieselbe wie bei R4 — eine 0 an dieser Stelle wuerde genau das fail-open wieder oeffnen, das der Slice schliesst. (2) Der Commit-Text und ein Kommentar nennen 26 korrigierte Test-Roster; tatsaechlich sind es 24 (20 Platzhalter, 3 plus 1 Veroeffentlichungs-Ids). Die Korrektur selbst ist vollstaendig — im ganzen Verzeichnis docs/testing/ ist kein Platzhalter und keine Veroeffentlichungs-Id mehr uebrig. Nur die genannte Zahl war um zwei zu hoch.
- Achse E (Design-Konformitaet), Bericht und PO-Nachmessung:

Zwei Abweichungen vom Plan, beide als gerechtfertigt beurteilt:
(1) Risiko R4 — die Generalisierung des fail-closed-Verhaltens wurde umgesetzt, obwohl design.md sie ausklammert. Der Pruefer bestaetigt unabhaengig, was hier bereits korrigiert vermerkt ist: das ist kein Alleingang des Implementierers, sondern AC4 des Kind-Issues 03, und der Widerspruch liegt zwischen zwei vom PO verfassten Artefakten. Dazu C4: der Plan sah fuer UNSUPPORTED_FIELD die Rueckgabe 0 vor, umgesetzt ist der Sentinel — dieselbe Begruendung, bereits als angenommene Abweichung vermerkt.
(2) Die Modulkarte des Plans fuehrt sechs Module als 'nicht beruehrt'.

Zu (2) nachgemessen, weil der Bericht vier verletzte Module nennt: das stimmt so nicht. Der Pruefer hat gegen den Merge-Base gediffed, und der liegt vor den Main-Issues 76, 77 UND 79 — alle drei teilen sich diesen Branch. Nur der Diff e5d9b3d..HEAD gehoert zu Main-Issue 77. Darin gilt:
  constraints.js              +44/-17   beruehrt
  report.js                   +81/-10   beruehrt
  countIndex.js               unveraendert
  resolver.js                 unveraendert
  catalogSet.js               unveraendert
  violationClassification.js  unveraendert
