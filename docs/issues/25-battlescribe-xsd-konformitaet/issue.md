...
- Child-Issue 13 hinzugefügt und umgesetzt: loadAllData() in App.jsx in loadLocalData() (awaited, lokal) und refreshCatalogInBackground() (nicht-blockierend, Netzwerk) aufgeteilt. handleSystemImported wartet nur noch auf den lokalen Reload vor dem Navigieren; der Katalog-Netzwerkabgleich läuft im Hintergrund weiter und aktualisiert systems/zeigt den Fehler-Toast bei Bedarf. Grund: Child-Issue 12s Flash-Fix hatte versehentlich den 'invisible by design'-Vertrag aus catalogUpdate.js gebrochen, indem er den ganzen Netzwerk-Katalogabgleich vor die Navigation gezogen hat. 13/13 Child-Issues resolved.
