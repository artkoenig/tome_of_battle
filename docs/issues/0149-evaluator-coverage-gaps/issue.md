...
- `docs/testing/parent-min-include-children-bolt-thrower` is green in every case, in particular the failing one `rosters/04-three-teams-max-fires.ros`. What the catalog data demands: a shared parent-scoped limit reports ONE violation for its counting frame, not one per counted selection. The Dark Elves unit "Reaper Bolt Thrower" (`a757-462a-11d5-9636`) holds three copies of the model entry "Reaper Bolt Thrower team" (`8d99-db74-0051-4a45`), whose max constraint `ccf9-fefc-71c8-bd73` (`max 2`, `field="selections"`, `scope="parent"`, `shared="true"`, `includeChildSelections="true"`) counts 3 in that one frame and must therefore fire exactly once with actual 3 against bound 2. The report carries three violations of that same limit instead of one. The sibling minimum `41ec-bee5-0865-0448` of the same pair already reports exactly once in the four other rosters, including the frame-separating roster 05 with two units, so the duplication is on the max side of this pair and not a general one.
