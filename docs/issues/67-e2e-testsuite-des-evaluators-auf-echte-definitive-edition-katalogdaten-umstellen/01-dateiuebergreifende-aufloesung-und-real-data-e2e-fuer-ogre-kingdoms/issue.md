...
Diese Slice befähigt die Engine, einen realen Armee-Katalog (`.cat`) samt
zugehöriger Spielsystemdatei (`.gst`) **vollständig aufzulösen** — `entryLink`,
`infoLink` und `sharedSelectionEntries` werden transitiv über die Link-Ketten
zusammengeführt, sodass importierte Definitionen tatsächlich mit-ausgewertet
werden. Ein Verweis, der auch nach der Auflösung nicht gefunden wird, erscheint
weiterhin als Diagnose, nie als Absturz.

Als erster Beleg wird der **echte** Ogre-Kingdoms-Katalog zusammen mit der
`.gst` end-to-end ausgewertet. Die realen Katalog-Rohdaten werden dafür an einem
Ort im Repository bereitgestellt, den die E2E-Tests der neuen Engine lesen
können, ohne die Clean-Room-Trennung (ADR-0030) zu verletzen und ohne die
bestehenden Testdaten der alten Engine anzutasten.
...
## Acceptance Criteria
- [ ] Wird ein echter Ogre-Kingdoms-Katalog zusammen mit seiner Spielsystemdatei
      ausgewertet, so werden per Verweis importierte Definitionen (einschließlich
      solcher aus der Spielsystemdatei) in der Auswertung berücksichtigt und
      erscheinen nicht fälschlich als unaufgelöst.
...
- [ ] Die echten Ogre-/Spielsystem-Rohdaten liegen versioniert im Repository und
      werden von den E2E-Tests gelesen, ohne die Testdaten der alten Engine zu
      verändern.
