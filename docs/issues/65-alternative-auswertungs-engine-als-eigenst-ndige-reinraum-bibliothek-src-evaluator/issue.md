...
- Architektur-Abgleich Slice 06: 1 blockierender Fund behoben – Grenze an der Kontingent-Definition (§3.2) las actual=0 selbst bei vorhandenem Kontingent, weil Force-Knoten nichts zum Index beitrugen. Fix: Force-Knoten zählen unter ihrer eigenen Definitions-ID (nicht null/Kategorien) – additiv, §3.3-treu; behebt Fehlalarm und macht Phantom-Absenz für Force-Typen korrekt. 2 Regressionstests. Sonst konform (Phantome zählen 0 via realNodes, Constraints über allNodes).
- Architektur-Abgleich Slice 07: konform, 0 Funde. SlotCapability §4.8-treu (Grenzfälle at-max→blocked, unter-min→mandatoryUnmet, kein-Max→headroom null verifiziert), selectableSlotsOf=reale+Pflicht-Phantomslots, Lookups reine Reads, Diagnosen vollständig. Berichtsform: {violations, capabilities(Map), diagnostics}. 108 Evaluator-Tests grün.
- Architektur-Abgleich Slice 08: 1 kritischer Fund behoben – INSTANCE_OF-Semantik. notInstanceOf war fälschlich als instanceOf mit value=0 modelliert; laut XSD/BSData und echter Fixture (type=notInstanceOf value=1) sind es zwei getrennte Operatoren. Fix: eigener NOT_INSTANCE_OF, instanceOf→actual>0, notInstanceOf→actual===0 (value-unabhängig) + Tests auf beide. Sonst konform: eigene Fixtures (nicht reused), E2E deckt Ogerbullen/min/max/Prozent/bedingten Modifikator, Smoke-Test echt (nicht hohl), Multi-Katalog ehrlich als Zukunft dokumentiert.
- Fünf-Achsen-Review grün. Standards: Gate besteht (Lint/Typecheck/Depcruise sauber), 3 kleine Funde – 2 behoben (node.def.limits ?? [] Guard; nicht-numerische Operation auf numerischem Ziel erzeugt jetzt Diagnose statt stillem No-op), 1 Data-Clump bewusst gelassen. Spec: keine fehlenden Anforderungen; 1 Fund (eigenes vs. echtes BS-Vokabular) als bekannte Grenze in README + ADR-0030 dokumentiert. Tests: grün (1716 Vitest + Puppeteer-E2E). Docs: kritischer Widerspruch behoben (depcruise ist für die Engine-Trennung blockierend, nicht mehr pauschal warn-only – CLAUDE.md/README/.dependency-cruiser.cjs korrigiert), README-Architektur um src/evaluator/ ergänzt. Alle Akzeptanzkriterien der 8 Slices erfüllt und abgehakt.
