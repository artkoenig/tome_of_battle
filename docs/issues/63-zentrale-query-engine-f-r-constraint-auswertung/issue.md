...
- Folge-Slices 06/07 + Gate abgeschlossen. Fünf-Achsen-Review (zweifach) grün: vitest 1589 + E2E, lint/typecheck sauber, knip/depcruise advisory. Behobene Findings: A1 (armeeweite Kategorie-Summen an einer Stelle, summierend statt spread-overwrite — Multi-Force-Unterzählung weg), B1 (Profil-Pfad reicht force+counts durch, force+Eintrags-Repeat zählt pro Kontingent wie die Validierung), A3/A6 (DRY: geteilte modifierConditionsPass/getModifierAmount, tote Exporte entfernt), Doku (Ziel-Typ-Regel in battlescribe-data-format.md/ADR 0003/0029/CONTEXT.md, stale Ref in ADR 0011, queryEngine.js in README). Bewusst zurückgestellt (Nicht-Blocker): A4 ctx-Builder/Datenklumpen (flacher Modifier-ctx ~9x gebaut; makeCtx reicht counts+Scheiben doppelt) → Kandidat für Folge-Issue; A2 instanceOf-checkInstance separat (boolescher Pfad, kommentiert); A5 Modul-Zyklus vorbestehend/dokumentiert.
- Slice 08 (auf Nutzerwunsch nach EINER Implementierung): der zweite Kategorie-Zähl-Eintrittspunkt (resolveCategoryAnchor/CATEGORY-Anker) ist entfernt; resolveScopeAnchor ist jetzt der alleinige Resolver auch fuer force-deklarierte Kategorie-Limits. Die kritische leere-Kategorie=0-Eigenschaft ist erhalten (isCategoryScope-Flag). Am Code verifiziert: keine resolveCategoryAnchor-Produktivstelle mehr, kein CATEGORY-AnchorKind. vitest 1591 + E2E grün, lint/typecheck/knip sauber. Verhaeltnismaessig abgeschlossen (self-verifizierter SSOT + gruene Vollsuite) statt drittem Full-Review.
