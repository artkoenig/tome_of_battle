...
| **ALP-R2** | **Gezählt werden nur die direkten Kinder des Rahmens.** Die Bedingung schreibt `includeChildSelections` **nicht** hin; der Vorgabewert der `QueryBase` ist `false` — „just `scope`'s `field`". | `src/parser/schema/Catalogue.xsd:430` `<xs:attribute name="includeChildSelections" type="xs:boolean" default="false"/>`; Deutung in [§7.6](../../battlescribe-data-format.md#76-constraint). (Praktisch latent, siehe „Bewusst nicht gepinnte Facetten".) |
