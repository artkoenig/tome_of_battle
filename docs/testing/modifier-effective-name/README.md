...
| `append` **ohne** `join` (Verkettung ohne Trennzeichen) | Einziges Vorkommen ist `Mercenaries`-`.cat` Z. 4817 (`<modifier type="append" value="*" field="name"/>`). Seine `modifierGroup` (Z. 4815–4841) hängt an verschachtelten `conditionGroup`s: einer `and`-Gruppe aus `lessThan … scope="force" childId="7d87-7436-5341-bbc0"` (Z. 4824) **und** einer `or`-Gruppe aus sieben `instanceOf … scope="primary-catalogue"` (Z. 4829–4835). Seit Issue 77 ist dieser Rahmen auflösbar (siehe [`primary-catalogue-scope`](../primary-catalogue-scope/README.md)), der Fall wäre also grundsätzlich herstellbar — nur nicht mit dem Datensatz dieses Szenarios in einem *minimalen* Roster: von den sieben genannten Katalogen liegen bloß Vampire Counts (`4d73-…`) und Orcs and Goblins (`4049-…`) im Ausschnitt, und beide Bedingungen müssten zugleich getroffen werden. Weiterhin bewusst ausgelassen, jetzt aus diesem Grund. |
