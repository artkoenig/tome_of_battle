...
- zusaetzlich im Datensatz: `Orcs and goblins (6th definitive edition).cat`
  (fuehrt als **einziges** Buch einen Wurzel-`entryLink` auf „Manbiters“ — seit
  Issue 0156 verankert der unter einem Vampirfuersten-Kontingent kein Angebot
  mehr) und
...
- **Angebots-Anker auf Wurzel-`selectionEntry`s** (Fell Bats, Dire Wolves) und
  ...
  `null`.
- **„Manbiters“ ist der Sonderfall — und faellt deshalb seit Issue 0156 ganz
  ...
  ausgewertet (Issue 0156): der O&G-Link verankert unter einem
...
- **Die Autor-Meldung an „Manbiters“.** Der Eintrag traegt einen
  ...
  0156 hat dieses Szenario ohnehin keinen Manbiters-Slot mehr, an dem etwas zu
  behaupten waere.
...
- **OCS-R2 (Kategorie-Filter).** In Roster 03 darf es **keinen** Slot mit
  ...
  `4fed…, 32f1…, 7a1c…, 0644…, d024…, c16b…, 64bf…, 43cc…, e94b…`. Eine
  maschinelle Gegenprobe gibt es dafuer seit Issue 0156 nicht mehr: in Roster 01
  traegt das `forceEntry` zwar den `categoryLink 6948-84bc-be26-e39a → ee09…`,
  der Kategorie-Filter liesse „Manbiters“ dort also durch — der Wurzel-`entryLink`
  darauf stammt aber aus der O&G-`.cat` und damit aus einem fremden Armeebuch,
  weshalb auch dort kein Slot entsteht. Das Manifest verlangt ihn in keinem
  Roster mehr.
...
| „Manbiters“ (nur Kategorie *Regiment of Renown*, Grenze `max 0 parent`; seit Issue 0156 in keinem Slot dieses Szenarios) | `0efb-7f63-7932-0655` — `categoryLink dc60-fb87-cc94-ad35 → ee09…`, constraint `30f0-d417-2185-bf4a` | Mercenaries-`.cat` → `<sharedSelectionEntries>` (Zeile 6552; Abschnitt 88–9525, also **nicht** wurzelnah) |
