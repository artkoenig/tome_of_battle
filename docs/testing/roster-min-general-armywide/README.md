...
| **RMGA-R8** | Die Mindestgrenze ist **nicht** durch `hidden` entwertet: Kategorie, geteilte Definition, `entryLink` und Trägereinheit sind alle `hidden="false"` und tragen keinen `hidden`-Modifikator. | Tabelle oben; [§5.6-Regelkasten](../../battlescribe/files/game-system.md#56-force-entries-detachments): *„dessen Mindestgrenzen dürfen **nicht** validiert werden"* — Voraussetzung liegt hier nicht vor. |
...
- **`actual` je General-Auswahl** = 1: jede Auswahl trägt `number="1"`, und die
  ...
  ([§7.5](../../battlescribe/building-blocks/cost.md#75-cost--cost-type)) ändert an
  ...
  ([§15](../../battlescribe/reference/source-gaps.md#15-lücken-der-quelle)).
