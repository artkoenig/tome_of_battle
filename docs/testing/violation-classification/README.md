...
- Datensatz-Override der Roster 06/07: `Vampire Counts (6th definitive edition).cat`
  ...
  (Roster 07 zusaetzlich mit der O&G-`.cat`, weil erst deren Wurzel-`entryLink`
  „Manbiters" ins Angebot bringt)
...
| **VCC-R10** | **Ein Pflicht-Anker meldet ganz normal.** „Army of Sylvania" ist ein Wurzel-Eintrag mit je einer unerfuellten `min 1`-Grenze in **force**- und in **parent**-Reichweite. Fehlt er im Kontingent, liefert er zwei abgeleitete Meldungen mit `anchorKind=mandatoryPhantom` und `anchorName="Army of Sylvania"`, die sich **nur** im `scopeKind` unterscheiden (`force` vs. `parent`) — der Beleg, dass `scopeKind` wirklich aus `constraint/@scope` und nicht aus dem Anker stammt. | VC-`.cat`, Wurzel-`<selectionEntries>` → `selectionEntry "Army of Sylvania"` **`b48b-4a69-80f1-5d47`** (`hidden="true"`) mit constraints **`1f2f-e5cc-d04d-162e`** (`min 1`, `scope="force"`) und **`e23f-0cea-11ac-9376`** (`min 1`, `scope="parent"`); die uebrigen beiden sind `max 1` (`e574-8cdb-9a8a-e48f` / `9f7d-8853-00c9-4bb1`). Der einzige Modifikator des Eintrags setzt `hidden` — **kein** Constraint-Modifikator, also `causes: []`. |
| **VCC-R11** | **Ein Angebots-Anker schweigt in der Meldungsliste — aber nicht im Faehigkeits-Datensatz.** „Manbiters" wird im Blood-Dragon-Kontingent nur **angeboten** (`anchorKind=offerAnchor`). Seine Autor-Meldung greift dort (der Schalter „Allow experimental rules?" ist nicht gewaehlt) und steht deshalb in `capabilities[].authorMessages` — aber **nicht** in der Meldungsliste. Ebenso wenig steuert ein Angebot eine abgeleitete Grenze bei. | Mercenaries-`.cat` → `selectionEntry "Manbiters"` **`0efb-7f63-7932-0655`** mit `modifier type="add" value="Please enable &quot;Allow experimental rules?&quot;" field="error"` und `condition type="lessThan" value="1" field="selections" scope="force" childId="8b76-92c4-23f9-54b1"`, dazu constraint **`30f0-d417-2185-bf4a`** (`max 0`, `scope="parent"`). Ins Angebot kommt der Eintrag ueber den O&G-Wurzel-`entryLink` `e3c2-1778-d3d5-edd1`; dass er dort ein Angebots-Anker ist, ist in [`offer-and-category-slots`](../offer-and-category-slots/README.md) (OCS-R2) belegt. Gegenprobe fuer die abgeleitete Haelfte: **`98c2-b213-2d60-6920`** (`min 3`, `scope="parent"`) am Modell `6dd9-c477-0549-37bb` unterhalb des ebenfalls nur angebotenen „Fell Bats" `a431-097d-4712-eb01` (OCS-R8). |
...
| 07 | Pflicht-Anker meldet, Angebots-Anker schweigt | Kontingent „Clan Blood Dragons" nur mit den beiden „Allow"-Schaltern. | **VCC-R10:** „Army of Sylvania" liefert **zwei** Meldungen am selben **Pflicht-Anker**, die sich nur im Bezugsrahmen unterscheiden (**Kontingent** vs. **Elternauswahl**), je Ist **0** von **1**, Differenz **+1**, ohne Ursache. **VCC-R11:** Der Angebots-Anker „Manbiters" traegt seine Autor-Meldung im Faehigkeits-Datensatz, aber die Meldungsliste enthaelt **weder** diese Autor-Meldung **noch** eine Grenze aus dem Angebot (`30f0…`, `98c2…` je **null**mal). | [`07-phantom-and-offer.ros`](rosters/07-phantom-and-offer.ros) |
...
| „Manbiters" (Angebots-Anker mit Autor-Meldung) + `max 0` | `0efb-7f63-7932-0655` — `30f0-d417-2185-bf4a` | Mercenaries-`.cat` → `<sharedSelectionEntries>` |
| Wurzel-`entryLink` auf „Manbiters" (bringt ihn ins Angebot) | `e3c2-1778-d3d5-edd1` → `0efb-7f63-7932-0655` | O&G-`.cat` → `<entryLinks>` |
