...
- Datensatz-Override der Roster 06/07: `Vampire Counts (6th definitive edition).cat`
  ...
  „Manbiters" zeigt — seit Issue 0159 verankert er unter einem
  Vampirfuersten-Kontingent allerdings kein Angebot mehr; der Angebots-Anker
  dieses Rosters ist stattdessen „Toxote's Hellmounts", auf den die
  Vampirfuersten-`.cat` einen **eigenen** Wurzel-`entryLink` fuehrt)
...
| **VCC-R11** | **Ein Angebots-Anker traegt seine Autor-Meldung im Faehigkeits-Datensatz und steuert NICHTS zur Meldungsliste bei.** Der Anker ist „Toxote's Hellmounts": eine geteilte Mercenaries-Definition, die die Vampirfuersten-`.cat` **selbst** per Wurzel-`entryLink` einbindet, im Roster aber nicht gewaehlt ist. Herleitung der gepinnten Zahlen: keine `min`-Grenze → `effectiveMin = null`, `isMandatoryUnmet = false`; genau eine eigene Grenze `max 0` und der einzige Modifikator darauf (`set 1`) verlangt „Allow experimental rules?" `8b76-92c4-23f9-54b1`, das im Roster fehlt → `effectiveMax = 0`; die Einheit steht nicht im Roster → `current = 0`, also `headroom = 0 − 0 = 0`. Dieselbe fehlende Auswahl erfuellt die `lessThan 1`-Bedingung der Autor-Meldung → sie steht mit `severity="error"` (aus `field="error"`) und dem Katalogtext im Faehigkeits-Datensatz. In der **Meldungsliste** erscheint sie trotzdem nicht, und `9595-a908-74bc-94bc` ebenso wenig. Ebenso wenig steuert ein Angebot eine abgeleitete Grenze bei: das zeigt die Gegenprobe am nur angebotenen „Fell Bats". | Mercenaries-`.cat` → `selectionEntry "Toxote's Hellmounts"` **`1a52-2060-f39b-38ee`** (Zeile 6362) mit `modifier type="add" value="Please enable &quot;Allow experimental rules?&quot;" field="error"` und `condition type="lessThan" value="1" field="selections" scope="force" childId="8b76-92c4-23f9-54b1"`, dazu constraint **`9595-a908-74bc-94bc`** (`max 0`, `field="selections"`, `scope="parent"`) und `modifier type="set" value="1" field="9595-a908-74bc-94bc"` mit `condition type="atLeast" value="1" … childId="8b76-92c4-23f9-54b1"`. Verankert im Kontingent durch den **VC-eigenen** Wurzel-`entryLink` **`d582-0c51-60be-ce82`** (VC-`.cat` Zeile 29641). Gegenprobe fuer die abgeleitete Haelfte: **`98c2-b213-2d60-6920`** (`min 3`, `scope="parent"`) am Modell `6dd9-c477-0549-37bb` unterhalb des ebenfalls nur angebotenen „Fell Bats" `a431-097d-4712-eb01` (OCS-R8). |
| **VCC-R12** | **Warum hier nicht mehr „Manbiters" steht.** Der Mercenaries-Eintrag „Manbiters" traegt denselben Bauplan (Autor-Meldung + `max 0`) und war bis Issue 0159 der Anker dieser Regel. **Kein** Vampirfuersten-Katalog deklariert jedoch einen Wurzel-`entryLink` darauf — der einzige steht in der **O&G**-`.cat` —, und ein Kontingent wird nur gegen sein eigenes Armeebuch, dessen `catalogueLink`-Huelle und das Spielsystem ausgewertet. „Manbiters" wird im Blood-Dragon-Kontingent also gar nicht mehr angeboten und kann dort keinen Anker mehr stellen; „Toxote's Hellmounts" dagegen wird vom eigenen Buch verlinkt (VCC-R11). | „Manbiters" **`0efb-7f63-7932-0655`** (Mercenaries-`.cat`, `<sharedSelectionEntries>`, constraint `30f0-d417-2185-bf4a`); einziger Wurzel-`entryLink` darauf: `e3c2-1778-d3d5-edd1` in der O&G-`.cat` (`<entryLinks>`, Zeile 14896). Die VC-`.cat` fuehrt in ihren Wurzel-`<entryLinks>` (29513–29656) keinen Verweis auf `0efb…`. Dass daraus kein Angebot mehr entsteht, haelt [`offer-and-category-slots`](../offer-and-category-slots/README.md) (OCS-R2/OCS-R11) fest. |
...
| 07 | Versteckter Pflicht-Anker schweigt, Angebots-Anker schweigt | Kontingent „Clan Blood Dragons" nur mit den beiden „Allow"-Schaltern. | **VCC-R10:** Der effektiv versteckte Pflicht-Anker „Army of Sylvania" erzeugt **keine** Meldung aus seinen beiden unerfuellten Min-Grenzen (`1f2f…`, `e23f…` je **null**mal; Issue 0088). **VCC-R11:** Der Angebots-Anker „Toxote's Hellmounts" traegt seine Autor-Meldung im Faehigkeits-Datensatz (0 von 0, kein Spielraum, keine Pflicht), die Meldungsliste enthaelt **weder** diese Autor-Meldung **noch** eine Grenze aus einem blossen Angebot (`9595…`, `30f0…`, `98c2…` je **null**mal). **VCC-R12:** „Manbiters" wird hier seit Issue 0159 gar nicht mehr angeboten und stellt darum keinen Anker mehr. | [`07-phantom-and-offer.ros`](rosters/07-phantom-and-offer.ros) |
...
| „Toxote's Hellmounts" (Autor-Meldung + `max 0`; der Angebots-Anker von Roster 07) | `1a52-2060-f39b-38ee` — `9595-a908-74bc-94bc` | Mercenaries-`.cat` → `<sharedSelectionEntries>` (Zeile 6362) |
| Wurzel-`entryLink` auf „Toxote's Hellmounts" (**eigenes** Armeebuch — verankert das Angebot im VC-Kontingent) | `d582-0c51-60be-ce82` → `1a52-2060-f39b-38ee` | VC-`.cat` → `<entryLinks>` (Zeile 29641) |
| „Manbiters" (derselbe Bauplan; seit Issue 0159 in diesem Kontingent kein Anker mehr) | `0efb-7f63-7932-0655` — `30f0-d417-2185-bf4a` | Mercenaries-`.cat` → `<sharedSelectionEntries>` |
...
| „Allow experimental rules?" (Schalter beider Autor-Meldungen) | `8b76-92c4-23f9-54b1` | `.gst` |
