...
| **REM-R5 (Pflicht-Stelle bei greifender Anhebung)** | Fehlt die geforderte Einheit **und greift die Anhebung**, ist ihr Auswahlpunkt **kein Angebot**, sondern eine **Pflicht-Stelle** mit `current 0`, effektiver Untergrenze 1 und unerfüllter Pflicht. Greift die Anhebung nicht (Ironskin Tribe), ist die Pflicht **nicht unerfüllt** (`isMandatoryUnmet false`) — welche **Ankerart** der Auswahlpunkt dann trägt, sagt diese Regel bewusst **nicht**. | Ableitung aus REM-R1/R2 plus der bereits gepinnten Kodierungs-Schwester: `offer-and-category-slots` führt den Wurzeleintrag „Army of Sylvania" (`b48b-4a69-80f1-5d47`, `min 1 scope=force`) als Pflicht-Stelle (`mandatoryPhantom`, `effectiveMin 1`, `isMandatoryUnmet true`). **Korrigiert am 2026-07-29:** die frühere Fassung schloss aus dem Wurzel-`entryLink` „Manbiters" (Ziel `0efb-7f63-7932-0655`), dass der Ironskin-Fall `offerAnchor` sein müsse. Der Schluss trägt nicht — „Manbiters" hat **gar keinen** `min`-Constraint, `32ed-…` hat einen, der nur auf 0 steht. Das sind verschiedene Fälle, und die Katalogdaten entscheiden nicht, wie die Engine den zweiten einordnet. Die Zusage ist deshalb entfallen; siehe Issue 0108. |
...
| 03 | Ironskin `8711-…` | — | **0** (Basis, nicht angehoben) | 0 ≥ 0 — **keine** Verletzung, und der Auswahlpunkt trägt **keine unerfüllte Pflicht** (`isMandatoryUnmet false`). Beleg, dass der Modifikator **des Links** gelesen wird. Die Ankerart bleibt hier ohne Zusage (Issue 0108). | [`03-ironskin-missing-bulls.ros`](rosters/03-ironskin-missing-bulls.ros) |
...
| 03 | — | `32ed-…` | im Rahmen `8711-…`: `current 0`, `isHidden false`, `isMandatoryUnmet false`, Name „Ogre Bulls" — **ohne** Zusage über die Ankerart (Issue 0108) |
| 04 | `32ed-…` 0/1 (count 1) | — | — |
| 05 | `32ed-…` 0/1 (count 1) | — | — |
