...
| `constraint` mit `shared="false"` + `scope="parent"` | Individuelles Limit für spezifische Eltern-Auswahlen (O&G / Vampire Counts). | `parent-min-unshared-unit-size` (Roster 04: zwei „Goblins"-Einheiten mit je 15 Modellen ergeben **zwei** Meldungen mit je Ist 15 gegen Grenze 20, nicht eine erfüllte Summe 30); Modultest `query.matrix`. Der Rahmen ist damit an echten Daten festgenagelt; dass `shared="false"` **darüber hinaus** nichts unterscheidet, weil `parent` ihm vorgeht (ADR 0003 §4), bleibt unverändert. | ✅ |
