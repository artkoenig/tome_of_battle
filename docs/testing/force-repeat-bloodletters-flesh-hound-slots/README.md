...
([§7.7](../../battlescribe/building-blocks/modifier.md#condition--eine-voraussetzung), ADR 0029).
...
([§7.6](../../battlescribe/building-blocks/constraint.md#76-constraint), Ziel-Typ-Regel). Mit
...
[§7.7, Kasten „Ein wiederholter `set` wächst nicht"](../../battlescribe/building-blocks/modifier.md#repeat--modifier-mehrfach-anwenden)).
...
|----|-------|--------------------------|
| **FRBFH-R1** | **Basis:** Die Flesh Hounds tragen genau **eine** Grenze: `max 0` Auswahlen, `field="selections"`, `scope="force"`, `shared="true"`, `includeChildSelections="false"`. `actual` ist die Zahl der Flesh-Hounds-Auswahlen **im Kontingent**, `bound` der wirksame `value`. Ohne jeden Modifikator ist der Eintrag also **gar nicht** wählbar. | `.cat` Z. 5886. Zählregel [§7.6](../../battlescribe/building-blocks/constraint.md#76-constraint): Eintragsziel bei `scope="force"` ⇒ pro Kontingent. |
...
- **`actual`** ist die Stückzahl der Flesh-Hounds-Auswahlen im Kontingent, also
  ...
  *„just `scope`'s `field`"*
  ([§7.6](../../battlescribe/building-blocks/constraint.md#76-constraint)) —, und sie sind
