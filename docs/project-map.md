...
| `src/evaluator/` | The "Reinraum" (clean-room) rule-evaluation engine — a pure function `evaluate(catalog, roster) → report`. The **only** production engine, successor to the deleted, faulty `src/solver/` ([ADR 0029](adr/0029-zentrale-query-engine-fuer-constraint-auswertung.md) → [0030](adr/0030-zweite-eigenstaendige-auswertungs-engine.md)). Deep reference: [`evaluator-architecture.md`](evaluator-architecture.md). Its own XML reader (`catalogReader.js`) is intentionally separate from `src/parser/`'s. |
| `src/evaluation/` | The bridge between the two: `rosterAdapter.js` translates the IndexedDB roster into the evaluator's input, `evaluationCache.js` memoizes the one `evaluateAppRoster` seam, `useEvaluation.js` is the hook the editor and play mode both use. |
| `src/components/` | React UI: `Importer`, `RosterDashboard`, `RosterEditor`, `PlayMode`, dialogs; subfolders `editor/`, `importer/`, `play/`. Most `.jsx` files are paired 1:1 with a `.test.jsx`. |
| `src/hooks/` | Focused, independently-testable hooks: `useRoster`, `usePlayState`, `useAppData`, `useAppNavigation`, `useUndoableState` ([ADR 0013](adr/0013-generischer-undo-redo-hook.md)), `useRuleUrl`, `useToast`, ... |
| `src/contexts/` | Only `SettingsContext.jsx` (whfb6 rule-linking toggle, [ADR 0015](adr/0015-settings-context-fuer-whfb6-verlinkung.md)). |
| `src/data/` | `rules-index.json` (generated by `scripts/generate-rules-index.js`) plus lookup/synonym matching to 6th.whfb.app rule pages ([ADR 0012](adr/0012-integration-externer-regeltexte-6th-whfb-app.md)). |
| `src/i18n/` | Home-grown i18n, no library ([ADR 0026](adr/0026-i18n-eigenloesung-json-und-intl-ohne-library.md)): `i18nStore.js`, `translate.js`, `locales/{de,en}.json`, `violationMessages.js` for evaluator report projection. |
| `src/utils/` | `rosterSerialization.js` (`.ros` export/import via JSZip), `createRoster.js`, `rosterDefaults.js`, `releaseDiff.js`. |
| `src/constants/` | Only `views.js`. |
| `src/styles/` | 33 numbered CSS layer files loaded in cascade order ([ADR 0004](adr/0004-styling-conventions.md) §6). |
| `src/test-utils/`, `src/__fixtures__/` | Shared test setup and sample catalogs (`generic/`, `whfb6/`, `whfb6-lexicanum/`). |

**The Reinraum boundary.** `src/evaluator/` and `src/roster/` must not import
each other in either direction, and `src/evaluator/evaluator.js` is the only
...
- **`evaluator-architecture.md`** — the reference architecture for
  `src/evaluator/` (pipeline stages, invariants); what
...
- **`testing/`** — evaluator E2E scenario fixtures, one subfolder per
  ...
  `src/evaluator/e2e.testcatalog.test.js`. Authored exclusively by the
...
2. **Evaluator E2E** — `src/evaluator/e2e.testcatalog.test.js` (plus
...
- **Codegen**: `generate-schema-module.js` (vendored `Catalogue.xsd` →
  `src/parser/schema/battlescribeSchema.generated.js`),
  `generate-rules-index.js` (→ `src/data/rules-index.json`),
  `rules-crawler.js`.
