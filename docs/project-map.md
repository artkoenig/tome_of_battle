...
|---|---|
| `src/contexts/armylist/application/` | The list-side data facade (ADR 0037, reclassified from Daten to Fachlogik by ADR 0040): `rosterStore.js`, `settings.js`, `rosterTransfer.js`. Reaches IndexedDB only through `../ports/storagePort.js`. Since Issue 0188 the editor's write use cases live here too — `raiseUnit.js`, `removeUnit.js`, `copyUnit.js`, `renameRoster.js`, `subSelectionUseCases.js` and `rosterSelectionFactory.js`: pure functions from roster to roster, with `system` and `report.slots` handed in. |
...
| `src/contexts/armylist/acl/` | The list context's **anti-corruption layer** (Issue 0191): the one seam that reads BattleScribe catalogue vocabulary for the editor and hands the UI our own words — `forceCategoriesOf`, `offerDefIdsOf`, `offerIdentifiesSlot`, `childOffersOf`, `childOfferCountOf`. Mapping rules in `catalogTranslation.js`; counterpart of `src/contexts/ruleengine/acl/rosterAdapter.js`. No module under `src/ui/` may name that vocabulary (`ui-kein-fremdformat`). |
...
| `src/contexts/play/` | The running game (issue 0190, [PRD](PRD-play-mode-eigener-kontext.md)): `model/game.js` (the `Game` aggregate — round, VP, CP, wounds per selection), `application/gameStore.js`, `ports/storagePort.js`, and `index.js` as the one door the UI uses. It references the list by `rosterId`; a wound no longer rewrites the roster record and no longer enters the list's undo history. Stored in its own object store `games`. |
