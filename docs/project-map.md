...
|---|---|---|
| UI | `src/ui/components/`, `src/ui/viewmodels/`, `src/ui/contexts/`, `src/ui/styles/`, `src/ui/i18n/` | Presentation and interaction |
...
|---|---|
| `src/data/services/` | The data facade (ADR 0037): `rosterStore.js`, `systemLibrary.js`, `settings.js`, `catalogRevisions.js`, `rosterTransfer.js`, plus the one change channel `dataEvents.js`. Every writing call announces its completion there; `src/ui/viewmodels/useAppData.js` is the single place that subscribes. |
...
| `src/ui/viewmodels/` | The ViewModel layer ([ADR 0038](adr/0038-custom-hooks-als-viewmodel-je-ui-baustein.md)): one ViewModel per screen, overlay and editor leaf (`editor/`), the roster state node (`useRosterState.js`), the two roster contexts, plus the app-level hooks `usePlayState`, `useAppData`, `useAppNavigation`, `useUndoableState` ([ADR 0013](adr/0013-generischer-undo-redo-hook.md)), `useRuleUrl`, `useToast`, `useRosterList`, `usePwaLifecycle`, `useViewportHeight`. Since Issue 0178 the layer has one directory. |
