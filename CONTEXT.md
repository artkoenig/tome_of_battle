...
**Listenkonfiguration**:
Ein listenweiter Battlescribe-Schalter, der über eine Armeeliste hinweg gilt, aber keine spielbare Einheit ist: eine `selectionEntry` vom Typ `upgrade`, deren gesamter Teilbaum (der Eintrag selbst und alle seine `selectionEntries`-Kinder) profil- und kostenlos ist, und die direkt an der Armeeliste hängt (Top-Level-Selection einer Force, nicht verschachtelt unter einer Einheit). Beispiel: „Allow experimental rules?" in der WHFB6 Definitive Edition.
