...
const PORT = 5177; // use a different port to avoid conflicts
const tempZipPath = path.resolve('./temp_verify_whfb6.zip');
...
const runVerification = async () => {
  ...
  try {
    ...
    // 1. Verify that R6 background texture is NOT present in body or app-content styles
    console.log('Verifying R6 background texture is fully reverted (removed)...');
    ...
    console.log('Body background-image style:', cssBackgroundData.bodyBg);
    console.log('App Content background-image style:', cssBackgroundData.appContentBg);

    const hasSvgTexture = (bgStyle) => {
      if (!bgStyle) return false;
      return bgStyle.includes('data:image/svg+xml');
    };

    if (hasSvgTexture(cssBackgroundData.bodyBg) || hasSvgTexture(cssBackgroundData.appContentBg)) {
      throw new Error('Verification failed: R6 background texture (SVG image) was found in styles.');
    }
    console.log('✅ Verified: No SVG background texture present on body or app-content. R6 is fully reverted.');
    ...
    await page.evaluate(() => {
      const btn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.toLowerCase().includes('armeeliste') || b.textContent.toLowerCase().includes('neu') || b.textContent.toLowerCase().includes('ausheben'));
      if (btn) btn.click();
    });

    await page.waitForSelector('form');
    await page.type('form input[type="text"]', 'Test Army for R5 Revert');
    ...
    await page.evaluate(() => {
      const btn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.toLowerCase().includes('spielmodus') || b.textContent.toLowerCase().includes('spielen'));
      if (btn) btn.click();
    });
    ...
    console.log('✅ Unit card has normal opacity and grayscale (unaffected).');
    ...
    // 3. Verify click pass-through: click the rules header underneath the overlay!
    ...
    console.log('🎉 ALL REVERTED MILESTONE 3 EMPIRICAL VERIFICATION CHECKS PASSED!');
  } finally {
