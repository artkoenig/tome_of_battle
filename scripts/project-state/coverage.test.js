...
describe('project-state/coverage', () => {
  describe('moduleFromPath', () => {
    ...
    it('normalizes backslash separators', () => {
      expect(moduleFromPath('C:\\repo\\src\\platform\\battlescribe\\read.js', 'C:/repo')).toBe('src/platform/battlescribe');
    });
  });
  ...
  describe('aggregateCoverage', () => {
    ...
    it('flattens per-branch hit arrays before counting', () => {
      const coverageFinal = {
        '/repo/src/platform/battlescribe/x.js': {
          s: {},
          b: { 0: [1, 0], 1: [1, 1] },
          f: {},
        },
    ...
    it('reports full coverage for a metric with no measurable entries (no 0/0 division)', () => {
      const coverageFinal = {
        '/repo/src/platform/battlescribe/empty.js': { s: {}, b: {}, f: {} },
      };
    ...
    it('sorts modules by name', () => {
      const coverageFinal = {
        ...
        '/repo/src/platform/battlescribe/a.js': { s: { 0: 1 }, b: {}, f: {} },
        '/repo/src/ui/components/a.js': { s: { 0: 1 }, b: {}, f: {} },
      };

      const modules = aggregateCoverage(coverageFinal, { rootPath: '/repo' }).map((entry) => entry.module);
      expect(modules).toEqual(['src/platform/battlescribe', 'src/solver', 'src/ui/components']);
    });
